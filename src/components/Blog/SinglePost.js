...
const SinglePost = (props) => {
  const { blogClass, blogID, blogImage, blogTitle, blogCategory, blogPublishedDate } = props;
  const shortenTitle = (title) => {
    return title.length > 60 ? title.substring(0, 80) + '...' : title;
  };
      ...
      <div className="blog__card--content">
        <div className="blog__card--content-area mb-25">
          ...
          <h3 className="blog__card--title">
            <Link to={`/berita/${blogID}`}>
              {blogTitle ? shortenTitle(blogTitle) : 'How to growing your business'}
            </Link>
          </h3>
        </div>
        <div className="blog__card--icon d-flex align-items-center">
          <div className="blog__card--icon-1">
            ...
            <span>{blogPublishedDate ? blogPublishedDate : 'April 18'}</span>
          </div>
        </div>
      </div>
    </div>
